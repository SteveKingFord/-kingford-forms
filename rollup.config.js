...
function createConfig(format, output, plugins = []) {
  ...
  const isCompatPackage = pkg.name === '@jin/compat'
  ...
  if (isGlobalBuild || isBrowserESMBuild || isCompatPackage) {
    ...
    external = [
      ...
      ...['path', 'url', 'stream'] // for @jin/compiler-sfc / server-renderer
    ]
  }

  // the browser builds of @jin/compiler-sfc requires postcss to be available
  ...
  const replacements = {
    ...
    __DEV__: isBundlerESMBuild
      ...
      : // hard coded dev/prod builds
        !isProduction,
    // this is only used during jin's internal tests
    __TEST__: false,
